...
console.log(`Sorry ${Guests}, Duo to some reason i just invite only two people`);

while(Guests.length > 2){
 let remove = Guests.pop()
 console.log(`Sorry ${remove}, you are no longer invited to the dinner`)
}

Guests.map((items) => console.log(`Congratulations ${items} you are still invited to the dinner`));
